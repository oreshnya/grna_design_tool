...
def calcRNADNAenergy(guideSeq, otSeq, GU_allowed=False):
    ...
    while i<epos:
        ...
        if loop_size<3:
            ...
            print(f"loop_size: {loop_size}")
            print(f"guideSeq[{i}:{j+1}]:", guideSeq[i:j+1])
            print(f"seq[{i}:{j+1}]:", seq[i:j+1])
            print(f"RNA_DNA.keys():", RNA_DNA.keys())
            print(f"RNA_DNA[{loop_size}].get({guideSeq[i:j+1]}):", RNA_DNA[loop_size].get(guideSeq[i:j+1]))
